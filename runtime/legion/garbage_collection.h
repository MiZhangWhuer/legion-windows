...
#define REFERENCE_NAMES_ARRAY(names)                \
    ...
    //--------------------------------------------------------------------------
    {
      p_fetch_and_add(&references,cnt);
    }
    ...
    //--------------------------------------------------------------------------
    {
      unsigned prev = p_fetch_and_sub(&references,cnt);
#ifdef DEBUG_LEGION
    ...
    //--------------------------------------------------------------------------
    {
      ...
      int previous = p_fetch_and_add(&gc_references, cnt);
#ifdef DEBUG_LEGION
      assert(previous >= 0);
#endif
      if (previous == 0)
...
#endif
#ifndef DEBUG_LEGION_GC
      int previous = p_fetch_and_add(&valid_references, cnt);
#ifdef DEBUG_LEGION
      assert(previous >= 0);
#endif
      if (previous == 0)
...
#endif
#ifndef DEBUG_LEGION_GC
      int previous = p_fetch_and_add(&resource_references, cnt);
#ifdef DEBUG_LEGION
      assert(previous >= 0);
#endif
      if (previous == 0)
    ...
    //--------------------------------------------------------------------------
    {
      ...
      while (current_cnt > 0)
      {
        ...
        const int prev_cnt = 
            p_atomic_cmpxchg(&valid_references, current_cnt, next_cnt);
