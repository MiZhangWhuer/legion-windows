...
#define REALM_THREAD_LOCAL __thread
    ...
    extern REALM_THREAD_LOCAL TaskContext *implicit_context;
    // Same thing for the runtime
    extern REALM_THREAD_LOCAL Runtime *implicit_runtime;
    // Another nasty global variable for tracking the fast
    // reservations that we are holding
    extern REALM_THREAD_LOCAL AutoLock *local_lock_list;
    ...
    extern REALM_THREAD_LOCAL::legion_unique_id_t implicit_provenance;
    ...
    extern REALM_THREAD_LOCAL unsigned inside_registration_callback;
    // Use this global variable to track if we're an
    // implicit top-level task that needs to do external waits
    extern REALM_THREAD_LOCAL bool external_implicit_task;
