...
namespace Legion {
  namespace Internal {
    ...
    IndexPartNode* RegionTreeForest::get_node(IndexPartition part,
                     RtEvent *defer/* = NULL*/, const bool can_fail/* = false*/)
    //--------------------------------------------------------------------------
    {
      if (!part.exists())
    ...
    //--------------------------------------------------------------------------
    {
      ...
      if (p_fetch_and_add(&invalidated, 1) > 0)
        return;
    ...
    //--------------------------------------------------------------------------
    {
      ...
      {
        for (FieldMaskSet<PartitionNode>::iterator it = 
              to_traverse.begin(); it != to_traverse.end(); it++)
    ...
    //--------------------------------------------------------------------------
    {
      const int remainder = p_fetch_and_add(&references, 0); 
#ifdef DEBUG_LEGION
