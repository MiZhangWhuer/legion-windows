...
namespace Legion {
  namespace Internal {
    ...
    //--------------------------------------------------------------------------
    {
      ...
      while (!done)
      {
        ...
        if (first)
        {
          ...
          assert(p_fetch_and_add(&gc_references, 0) > 0);
#endif
      ...
      while (!done)
      {
        ...
        if (first)
        {
          ...
          if (has_gc_references && 
              (p_fetch_and_add(&gc_references, 0) == 0))
...
#ifdef DEBUG_LEGION
          // Should have at least one reference here
          assert(p_fetch_and_add(&valid_references, 0) > 0);
#endif
      ...
      while (!done)
      {
        ...
        if (first)
        {
          ...
          if (has_valid_references &&
              (p_fetch_and_add(&valid_references, 0) == 0))
...
#ifdef DEBUG_LEGION
      // Should have at least one reference here
      assert(p_fetch_and_add(&resource_references, 0) > 0);
#endif
    ...
    //--------------------------------------------------------------------------
    {
      AutoLock gc(gc_lock);
      // Check to see if we lost the race for changing state
      if (p_fetch_and_add(&resource_references, 0) == 0)
      {
