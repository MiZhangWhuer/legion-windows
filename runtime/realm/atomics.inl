...
namespace Realm {
  ...
  inline T atomic<T>::load(void) const
  {
    ...
    //return p_fetch_and_add(const_cast<T *>(&value), 0);
#endif
  }
  ...
  inline T atomic<T>::load_acquire(void) const
  {
    ...
    //T val = p_fetch_and_add(const_cast<T *>(&value), 0);
#if _UNIX
    __sync_synchronize();
    ...
    OSMemoryBarrier();
#endif  // full memory fence is all we've got
    return val;
#endif
  }
  ...
  inline T atomic<T>::exchange(T newval)
  {
    ...
    while(true) {
      ...
      T oldval = value;
#endif
      if(compare_and_swap_64(&value, oldval, newval))
	return oldval;
    }
#endif
  }
...
#else
    return p_fetch_and_add(&value, to_add);
#endif
  }
...
#else
    return p_fetch_and_sub(&value, to_sub);
#endif
  }
